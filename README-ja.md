...
テンプレートには、Maven と Gradle のワークフローがあります。
どちらのワークフローも、次の処理を実行します。

- JDK 25 と JDK 26 でアプリケーションをビルドし、テストする
- GraalVM JDK 25 でネイティブ実行ファイルをビルドする

Maven のワークフローは、リポジトリシークレット `SONAR_TOKEN` が設定されている場合に、JDK 25 のビルド後に SonarQube スキャンも実行します。
SonarQube の設定は `pom.xml` にあり、Gradle のワークフローではスキャンを実行しません。
